...
void PointTracker::AddPoint(cv::Point &point) {
	if (this->reference) {
		...
		this->points[absolutePointNumber] = TrackedObject(point, absolutePointNumber++, 0);
	}
	else {
		this->points[absolutePointNumber] = TrackedObject(point, absolutePointNumber++, 0);
	}
...
RectangleTracker::RectangleTracker(unsigned int threshold): PointTracker(threshold) {}
RectangleTracker::RectangleTracker(unsigned int threshold, PointTracker reference): PointTracker(threshold, reference) {}

void RectangleTracker::AddRectangle(cv::Rect &rectangle) {
	cv::Point gravity_center = this->GetRectangleCenter(rectangle);
	this->AddPoint(gravity_center);
}	
